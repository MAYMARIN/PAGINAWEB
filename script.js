...
function generarRespuestaBot(mensaje) {
  ...
  if (texto.includes('hola') || texto.includes('buenas') || texto.includes('que tal')) {
    return "¡Hola! ¿Cómo estás? ¿como puedo ayudarte";
  } 
  else if (texto.includes('precio') || texto.includes('costo')) {
    return "Nuestros servicios tienen precios variados. contactanos via whatsapp";
  } 
  else if (texto.includes('horario') || texto.includes('abierto')) {
    return "Atendemos de Lunes a sabado de 9:00 am a 5:00 pm. los domingos no abrimos";
  } 
  else if (texto.includes('contacto') || texto.includes('telefono')) {
    return "Puedes escribirnos o llamarnos al 2215325025.";
  } 
